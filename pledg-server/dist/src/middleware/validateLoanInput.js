...
const validateLoanInput = (req, res, next) => {
    ...
    const errors = [];
    if (!loanAmountInr)
        errors.push('loanAmountInr is required');
    if (!interestRate)
        errors.push('interestRate is required');
    if (!ltv)
        errors.push('ltv is required');
    if (!durationInDays)
        errors.push('durationInDays is required');
    if (!collateralToken)
        errors.push('collateralToken is required');
    if (!collateralAmount)
        errors.push('collateralAmount is required');
    if (!borrowerId)
        errors.push('borrowerId is required');
    if (errors.length > 0) {
        res.status(400).json({
...
//# sourceMappingURL=validateLoanInput.js.map